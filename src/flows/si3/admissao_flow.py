...
class AdmissaoFlow:
    """Esqueleto de AdmissaoFlow — mantido para retrocompatibilidade dos testes unitários."""
    ...
    def execute(self, ctx: FlowContext, observer=None) -> FlowResult:
        ...
        for step_fn in [self._step_a01, self._step_a02]:
    ...
    def _step_a01(self, ctx, observer=None) -> StepResult:
        step_id = "A01"
        start = time.monotonic()
        try:
            ctx.runner.screenshot(f"{ctx.evidence_dir}A01.png")
    ...
    def _step_a02(self, ctx, observer=None) -> StepResult:
        step_id = "A02"
        start = time.monotonic()
        try:
            ctx.runner.screenshot(f"{ctx.evidence_dir}A02.png")
            step = StepResult(step_id=step_id, success=True,
                              duration_ms=(time.monotonic() - start) * 1000)
        except Exception as e:
            step = StepResult(step_id=step_id, success=False,
