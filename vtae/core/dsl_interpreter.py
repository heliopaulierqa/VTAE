...
from vtae.core.exceptions import RunnerError, StepError
...
class DSLInterpreter:
    ...
    def _action_login(self, index: int, step: dict) -> str | None:
        ...
        try:
            if sistema == "si3":
                ...
                from vtae.flows.sislab.login.login_flow_sislab import LoginFlowSisLab as LoginFlow
            elif sistema == "msi3":
                from vtae.flows.msi3.login.login_flow_msi3 import LoginFlowMsi3 as LoginFlow
            else:
                raise StepError(
                    f"Sistema '{sistema}' sem LoginFlow registrado no dsl_interpreter."
                )
    ...
    def _ocr_read(self, region: list[int] | None) -> str:
        # NOTA (achado durante migração módulo 4, 28/07/2026): esta chamada usa
        # assinatura incompatível com o OcrHelper real (ler_regiao/ler_tela_inteira
        # exigem screenshot_path como primeiro argumento). Zero cobertura de teste —
        # todo teste mocka _ocr_read diretamente. Pendência registrada, não corrigida
        # aqui (fora do escopo da migração — exigiria capturar screenshot antes).
        try:
            from vtae.vision.ocr_helper import OcrHelper
        except ImportError as exc:
