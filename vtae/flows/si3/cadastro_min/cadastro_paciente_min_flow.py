# vtae/flows/si3/cadastro_min/cadastro_paciente_min_flow.py
"""
...
from vtae.core.estado_jornada import salvar as _salvar_estado_jornada
from vtae.core.result import FlowResult, StepResult
from vtae.flows.base_flow import BaseFlow
from vtae.vision.ocr import OcrHelper
...
class CadastroPacienteMinFlow(BaseFlow):
    ...
    def execute(self, ctx, dados: dict = None, observer=None) -> FlowResult:
        ...
        for step_fn in [
            lambda: self._step_cm01_abrir(ctx, observer),
            lambda: self._step_cm02_pesquisar(ctx, dados, observer),
            lambda: self._step_cm03_novo(ctx, observer),
            lambda: self._step_cm04_nome(ctx, dados, observer),
            lambda: self._step_cm05_data_nascimento(ctx, dados, observer),
            lambda: self._step_cm06_sexo(ctx, dados, observer),
            lambda: self._step_cm07_nacionalidade(ctx, dados, observer),
            lambda: self._step_cm08_cor_etnia(ctx, dados, observer),
            lambda: self._step_cm09_gerar_matricula(ctx, observer),
            lambda: self._step_cm10_sair(ctx, observer),
        ]:
            step = step_fn()
    ...
    # CM01 — Abrir modulo via Localizar no Menu
    # ------------------------------------------------------------------
    def _step_cm01_abrir(self, ctx, observer=None) -> StepResult:
        def fn():
            self._focar_si3()
    ...
    # CM02 — Pesquisar nome (tela Parametros de Pesquisa)
    # ------------------------------------------------------------------
    def _step_cm02_pesquisar(self, ctx, dados, observer=None) -> StepResult:
        def fn():
    ...
    # CM03 — Clicar Novo (tela Pacientes — lista)
    # ------------------------------------------------------------------
    def _step_cm03_novo(self, ctx, observer=None) -> StepResult:
        def fn():
    ...
    # ------------------------------------------------------------------
    def _step_cm04_nome(self, ctx, dados, observer=None) -> StepResult:
        _ocr = [None]
    ...
    # ------------------------------------------------------------------
    def _step_cm05_data_nascimento(self, ctx, dados, observer=None) -> StepResult:
        _ocr = [None]
    ...
    # ------------------------------------------------------------------
    def _step_cm06_sexo(self, ctx, dados, observer=None) -> StepResult:
        _ocr = [None]
        _jab = [None]
        ...
        def fn():
            ...
            x, y = ctx.objects.coord("campo_sexo_lov")
            pyautogui.click(x, y)
            time.sleep(0.5)
            pyautogui.press("backspace", presses=10, interval=0.02)
    ...
    # ------------------------------------------------------------------
    def _step_cm07_nacionalidade(self, ctx, dados, observer=None) -> StepResult:
        _ocr = [None]
        _jab = [None]
        ...
        def fn():
            ...
            self._verificar_via_jab_se_mapeado(
                ctx, "campo_nacionalidade", nacionalidade, "CM07", _jab
            )
    ...
    def _selecionar_em_lov(self, ctx, campo_coord: str,
    ...
    def _preencher_popup_brasileiro(self, ctx, dados) -> None:
        """
        Popup Brasileiro:
    ...
    def _preencher_popup_estrangeiro(self, ctx, dados) -> None:
        """
        Popup Estrangeiro:
    ...
    def _preencher_popup_naturalizado(self, ctx, dados) -> None:
        """
        Popup Naturalizado:
    ...
    def _verificar_popup_erro_incor(self, ctx, nacionalidade: str) -> None:
        """
    ...
    # ------------------------------------------------------------------
    def _step_cm08_cor_etnia(self, ctx, dados, observer=None) -> StepResult:
        _ocr = [None]
        _jab = [None]
        ...
        def fn():
            ...
            self._verificar_via_jab_se_mapeado(ctx, "campo_cor_etnia", cor_etnia, "CM08", _jab)
    ...
    # CM10 — Sair para o Menu Principal
    # ------------------------------------------------------------------
    def _step_cm10_sair(self, ctx, observer=None) -> StepResult:
        def fn():
            self._focar_si3()
    ...
    # ------------------------------------------------------------------
    # Helper — verificacao estrutural via pyjab (Fase 0 — 3 camadas)
    # O jab_name e um LOCATOR do objeto (objects/cadastro_min.yaml),
    # nao um dado do flow — Modelo de Elemento universal (Projeto v1 §4).
    # Nao mapeado ainda -> pula com aviso (mesmo bootstrap das regioes_ocr).
    # Conexao lazy cacheada em ctx.jab — padrao AB07, validado 3x.
    # ------------------------------------------------------------------
    def _verificar_via_jab_se_mapeado(
        self, ctx, nome_objeto: str, valor_esperado: str, step_id: str,
        jab_holder: list = None,
    ) -> None:
        jab_name = ctx.objects.jab_name(nome_objeto)
        if not jab_name:
            # Bootstrap: objeto ainda nao calibrado — tolerante, mas VISIVEL.
            msg = (f"[{step_id}] AVISO: objeto '{nome_objeto}' sem jab_name "
                   f"mapeado — verificacao pyjab pulada (bootstrap).")
            _log = getattr(ctx, "_logger", None)
            if _log:
                _log.warning(msg)
            else:
                print(msg)
        ...
        # A partir daqui o objeto TEM jab_name: a verificacao e OBRIGATORIA.
        # Nao poder executa-la e defeito, nao bootstrap — falha o step.
        if not getattr(ctx, "jab", None):
            titulo = ctx.objects.titulo_jab()
            if not titulo:
                raise AssertionError(
                    f"[{step_id}] objeto '{nome_objeto}' tem jab_name mapeado, "
                    f"mas 'tela.titulo_jab' nao esta definido no YAML de objetos."
                )
            try:
                # JAVA_HOME de mentira (lado cliente pyjab) — vem do .env via
                # config, setado ANTES do import (a DLL e procurada no import).
                _jab_home = ctx.config.DADOS.get("jab_home_fake")
                if _jab_home:
                    os.environ["JAVA_HOME"] = _jab_home
                from pyjab.jabdriver import JABDriver
                ctx.jab = JABDriver(title=titulo)
            except Exception as e:
                msg = (f"[{step_id}] AVISO: JABDriver nao conectou na janela "
                       f"'{titulo}': {e} — verificacao pyjab pulada.")
                _log = getattr(ctx, "_logger", None)
                if _log:
                    _log.warning(msg)
                else:
        ...
        _holder = jab_holder if jab_holder is not None else [None]
        self._verify_campo_via_jab(ctx, jab_name, valor_esperado, step_id, _holder)
