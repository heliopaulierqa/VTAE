...
class LoginFlow:
    """
    Fluxo de login para SI3 (Oracle Forms desktop).
    ...
    def _step_usuario(self, ctx, observer=None) -> StepResult:
        ...
        try:
            ctx.runner.safe_click("templates/si3/login/campo_usuario.png")
    ...
    def _step_senha(self, ctx, observer=None) -> StepResult:
        ...
        start = time.monotonic()
        try:
            ctx.runner.safe_click("templates/si3/login/campo_senha.png")
    ...
    def _step_entrar(self, ctx, observer=None) -> StepResult:
        step_id = "L03"
        if observer:
            observer.log_step_start(step_id, "clicar no botão Entrar")
        start = time.monotonic()
        try:
            ctx.runner.safe_click("templates/si3/login/btn_entrar.png")
