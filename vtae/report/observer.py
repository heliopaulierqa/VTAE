...
class ExecutionObserver:
    ...
    def log_step_result(self, step: StepResult) -> None:
        ...
        if step.validated is True:
            ...
            msg += " | validado=False"
        # Fora do if de validated: valor lido e evidencia, independe de o
        # step ter sido marcado como validado.
        if step.ocr_lido:
            msg += f" | ocr_lido: '{step.ocr_lido}'"
        if step.jab_lido:
            msg += f" | jab_lido: '{step.jab_lido}'"
    ...
    def report(self, ctx) -> str:
        ...
        # ── HTML ──────────────────────────────────────────────────────
        try:
            from vtae.report.report_generator import generate
